...
├── data/
│   ├── raw/                        # Blender scene files (.blend) — GENERATED by
│   │                               # scripts/build_scene.py, gitignored (D-022)
...
- **`data/blender_scenes/eval_holdout/` is read-only after Phase 4.** Nothing
  in `src/` or `scripts/` should ever write to it. If a script needs to, that's
  a sign it's touching the wrong directory.
- **`data/raw/` holds the main-study `.blend`, which is a build artifact.**
  Originally this directory was described as hand-built scene files "never
  edited by scripts"; since the Phase 4 scene is built procedurally
  (`scripts/build_scene.py` from `configs/scenes/final_scene.yaml`, per D-022
  and the scene brief's reproducibility requirement), the `.blend` is
  *generated* and is gitignored. The config, not the `.blend`, is the source
  of truth — never hand-edit the `.blend` in the GUI and expect it to stick,
  and never treat it as the authoritative description of the scene.
